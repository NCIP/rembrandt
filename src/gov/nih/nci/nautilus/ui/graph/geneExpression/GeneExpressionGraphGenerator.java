		...
		if (resultant != null) {
			...
			if (resultsContainer == null) {
				...
				for (int i = 0; i < diseaseTypes.length; i++) {
					...
					if (diseaseName
							.equalsIgnoreCase(gov.nih.nci.nautilus.constants.NautilusConstants.ASTRO)) {
					...
					for (Iterator reporterIterator = reporters.iterator(); reporterIterator
						...
						if (diseaseResultset.getType().getValue().toString()
								.compareTo(NautilusConstants.NORMAL) == 0) {
					...
					try {
						...
						errors
								.add(
										ActionErrors.GLOBAL_ERROR,
										new ActionError(
												"gov.nih.nci.nautilus.ui.graph.geneExpression.GraphError"));
					}
				} else {
                    errors.add(ActionErrors.GLOBAL_ERROR,
                            new ActionError("gov.nih.nci.nautilus.ui.graph.geneExpression.GraphError"));
