...
public class QuickSearchAction extends DispatchAction {
	...
	private ActionForward doGeneExpPlot(ActionMapping mapping, ActionForm form,
		...
		Resultant resultant = ResultsetManager
				.executeGeneExpressPlotQuery(geneQuery);
		if (resultant != null) {
			...
			if (resultsContainer == null) {
				...
				ActionError ae = new ActionError(
						"gov.nih.nci.nautilus.struts.action.quickSearch.badgene");
			...
			if (resultsContainer instanceof GeneExprDiseasePlotContainer) {
				...
				System.out.println("set intValuesArray: [" + probeSetSize
						+ "][" + diseaseSize + "]");
				...
				for (Iterator diseasesIterator = diseases.iterator(); diseasesIterator
					...
					//groups.add(diseaseName); // add the string to the
					// array list
					if (diseaseName
							.equals(gov.nih.nci.nautilus.constants.Constants.NORMAL)) {
						groups[icounter] = diseaseName.substring(1); //snip
						// the
						// z
						// off
					} else {
					...
					for (Iterator reporterIterator = reporters.iterator(); reporterIterator
						...
						intValuesArray[counter][icounter] = intensityValue
								.doubleValue();
						System.out.println("intValuesArray[" + counter + "]["
								+ icounter + "] = "
				...
				if (groups.length > 0 && probeSets.length > 0
					...
					try {
						...
						//double[][] data= new double[][]{ { 250, 45, 36,
						// 66, 22 }, { 150, 15, 6, 62, 21 }, { 20, 145, 36,
						// 6, 33 }, { 250, 45, 36, 66, 57 }, { 150, 15, 6,
						// 62, 12 }, { 20, 145, 36, 6, 29 } };
						double[][] data = new double[probeSets.length][groups.length];
						System.out
								.println("creating data array size: ["
										+ probeSets.length + "]["
										+ groups.length + "]");
						...
						System.out.println("there are: " + paints.length
								+ " colors");
						System.out.println("should be: " + probeSetSize);
						...
						request.getSession(true).setAttribute("chart",
								axisChart);
					} catch (Exception e) {
